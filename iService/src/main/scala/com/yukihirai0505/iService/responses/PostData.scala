...
case class PostEntryData(PostPage: Seq[PostPage])

object PostEntryData {
  implicit val PostEntryDataFormat = JsonNaming.snakecase(Json.format[PostEntryData])
}

case class PostData(entryData: PostEntryData)
